...
export default {
  ...
  methods: {
    ...
    getBrandsInfo () {
      ...
      this.$http.get(api.ADVERTISEMENT + '/adInfo?id=' + this.form.id).then(data => {
        this.form = data.data
        if (this.form.image_url) {
          this.form.image_url = /^http/i.test(this.form.image_url) ? this.form.image_url : api.QiniuDomain + this.form.image_url
          this.adChanged = false
        }
        this.posChange(this.form.ad_position_id)
        this.adCroppa.refresh()
