...
export default {
  ...
  methods: {
    getAddressInfo () {
      this.$http.get(api.ADDRESS + '/info?id=' + this.form.id).then((data) => {
        this.form = data.data
        this.getProvincesList()
        this.getCitiesList()
        this.getDistrictList()

        if (this.form.user.nickname) {
          this.getUserRemote(this.form.user.nickname)
        }
      }).catch(err => {
    ...
    getCitiesList () {
      this.$http.get(api.REGION + '?type=2&pid=' + this.form.province_id).then((data) => {
        this.cities = data.data
        this.citiesBak = data.data
      })
    },
    getDistrictList () {
      this.$http.get(api.REGION + '?type=3&pid=' + this.form.city_id).then((data) => {
        this.districts = data.data
        this.districtsBak = data.data
      })
    },
    proviceChange () {
      this.cities = this.citiesBak.slice(0)
      this.cities = this.cities.filter(item => {
        return this.form.province_id === item.parent_id
      })
    },
    cityChange () {
      this.districts = this.districtsBak.slice(0)
      this.districts = this.districts.filter(item => {
        return this.form.city_id === item.parent_id
      })
    },
    save () {
