<template>
  <main-panel title="添加商品" backPath="/products">
    <el-tabs v-model="activeName">
      ...
      <el-tab-pane label="规格组合" name="combination" :disabled="!form.id">
        <el-row :gutter="24">
          <el-col :span="10" :xs="24" :sm="24" :md="10" :lg="10" style="margin-bottom: 20px;">
            <el-button type="success" icon="el-icon-plus" size="small" @click="newSpec" style="margin-bottom: 20px;">新增规格类型</el-button>
    ...
    <el-dialog title="新增规格类型" :visible.sync="dialogFormVisible" width="30%">
      <el-form :model="addSpec" label-width="80px" size="mini">
...
export default {
  ...
  methods: {
    ...
    getGoodsSpecs () {
      this.tbloading = true
      this.$http.get(api.SPECIFICATION + '/goods?id=' + this.form.id).then(data => {
      ...
      this.$refs.form.validate(async (valid) => {
        if (valid) {
          ...
          if (this.imgCroppa1.hasImage() && this.img1Changed) {
            ...
            this.form.gallery.push(res.key)
          }
